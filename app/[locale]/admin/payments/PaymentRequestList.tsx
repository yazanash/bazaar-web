...
import { Search, ExternalLink, Mail, CreditCard, Package } from "lucide-react";
...
export default function PaymentRequestsList({
  ...
  const t = useTranslations("admin.paymentRequests");
  ...
  const handleAction = async (type: "accepted" | "rejected") => {
    ...
    try {
      ...
      if (res.success) {
        ...
        alert(type === "accepted" ? t("accepted") : t("rejected"));
      }
    } catch (error) {
      console.error(error);
    } finally {
