...
import { InfiniteUserAdsList } from "./InfiniteUserAdsList";
export default async function MyAdsPage() {
  const walletResult = await getUserWallet();
  const wallet = walletResult.data;
  const response = await getUserAds(1);
  ...
  const initialItems = adsData?.items || [];

  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 pb-24 relative" dir="rtl">
      <div className="flex flex-row items-center justify-between mb-3">
        <h2 className="text-xl font-black text-blue-900 text-center">إعلاناتي</h2>
        <Link
          href="/new-ad"
      ...
      {initialItems.length === 0 ? (
        ...
        <InfiniteUserAdsList initialItems={initialItems} />
      )}
    </div>
  );
}