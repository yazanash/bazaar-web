...
export function BasicInfoCard({ data }: { data: any }) {
  return (
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3 text-slate-500 font-bold text-sm">
              <span className="flex items-center gap-1"><MapPin size={14}/> {data.city?.arabicName}</span>
              <span className="flex items-center gap-1"><Calendar size={14}/> {data.manufactureYear}</span>
            </div>
          </div>
          <div className="flex gap-2">
            <button className="p-2 bg-slate-50 rounded-full hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors">
              <Heart size={20} fill={data.isFavorite ? "currentColor" : "none"} />
            </button>
        ...
        <div className="flex justify-between items-end mt-6">
          ...
          <div className="text-[10px] font-bold text-slate-400 flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-lg">
            <Clock size={12} /> نُشر منذ فترة وجيزة
          </div>
        </div>
      </div>
    </section>
