...
export default function WalletCard({ UserWallet }: UserWalletProps) {
  const formattedDate = new Date(UserWallet.expiryDate).toLocaleDateString(
    "ar-SY",
    {
      month: "numeric",
      day: "numeric",
  ...
  return (
    ...
    >
      <div className="mb-5 bg-white border border-slate-100 rounded-2xl p-4 flex items-center shadow-sm hover:border-blue-100 transition-colors relative group">
        <div className="shrink-0 w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600 relative">
          <Wallet size={20} />
        </div>
